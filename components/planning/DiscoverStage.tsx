...
export function DiscoverStage() {
  ...
  const handleRefine = async (e?: React.FormEvent) => {
    ...
    await discoverDestinations(store.userInput, refinement);
    setRefinement('');
  };

  const handleCompare = () => {
    store.setStage(PlanningStage.SHORTLIST);
  };

  const handleShortlist = (dest: Destination) => {
    if (store.isInShortlist(dest.id)) {
  ...
  return (
    <div className="min-h-dvh pb-28">
      <div className="sticky top-0 z-30 bg-[#0A0F1E]/90 backdrop-blur-xl border-b border-white/5 px-4 py-3">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-3">
            <button
              onClick={() => store.setStage(PlanningStage.SPARK)}
      ...
      <div className="max-w-4xl mx-auto px-4 pt-6">
        ...
        >
          {store.discoveredDestinations.length > 0
            ? 'Here are 4 destinations that match your vibe'
        ...
        {store.discoveredDestinations.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {store.discoveredDestinations.map((dest, i) => (
              <DestinationCard
                key={dest.id}
                destination={dest}
                index={i}
                isShortlisted={store.isInShortlist(dest.id)}
        ...
        {store.shortlistedDestinations.length === 0 && store.discoveredDestinations.length > 0 && (
      ...
      <ShortlistTray shortlist={store.shortlistedDestinations} onCompare={handleCompare} />
    </div>
  );
}
